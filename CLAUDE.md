...
- **Orbs (the one moving element):** journey-step nodes are **dot-lattice globes** — dense latitude-ring lattice, vanilla 2D canvas, in the spirit of Jakub Antalik's MIT *thinking-orbs* (reimplemented, not imported; credit him in the README). Grey = ahead (still), blue = current (slow rotation), gold = done (still). They appear on the map, tiny in deck headers, and beside the brand in the hero — nowhere else; `prefers-reduced-motion` freezes them. This is the site's entire motion budget.
- **References:** `demos/wireframe-brilliant-light.html` (theme/type), `demos/wireframe-flashcards.html` (deck), `demos/wireframe-components.html` (buttons/icons/parts), `demos/wireframe-orbs.html` (orbs) — all approved by Sangam. `demos/guide-particles-preview.html`, `demos/humanoid-avatar-demo.html`, and `js/particles.js` are historical v1 artifacts.
