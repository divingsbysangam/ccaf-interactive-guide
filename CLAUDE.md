...
- **Brand mark / favicon:** `assets/mark.svg` is a solid safety-orange tile with a white three-tread staircase — square corners, flat fill, no gradient or glow. It replaced the v1 "journey constellation" (dark radial gradients, blue/gold glow, 14px radius) on 2026-08-20; plan R16's line about the constellation surviving is out of date. Every coordinate is a multiple of 4 so the mark rasterises crisp at 16 and 32px. The PNG fallbacks are generated — never hand-edit them, run `node scripts/build-icons.js`.
