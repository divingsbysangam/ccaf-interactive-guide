...
- **Form:** square corners (`--radius` 0–4px), 1px hairline rules instead of boxes wherever possible, 4px spacing base, **no drop shadows, no gradients, no glass**. Chrome is nav → subdomain bar (chalk band, mono host in safety) → content at `--container-lg` with a 720px reading measure → inverse insight band as the footer. The footer carries exactly three things: the field-note quote, the "last verified" line (R15), and Sangam's socials — LinkedIn / X / Substack / YouTube, rendered from `CCAF_SOCIALS` in `js/app.js` with the filled brand marks from `js/icons.js`. The old "Free & open — no login…" and "Bengaluru · Field System 1.0" lines were removed on Sangam's instruction (2026-08-20) — don't reinstate them. Brand marks are the one filled exception to the hairline icon rule, and each is always paired with its mono text label.
- **Brand mark / favicon:** `assets/mark.svg` is **"the crossing"** — an orb meeting a waterline on a safety-orange tile. Above the line the orb is solid white; below it, it is **offset sideways and dissolved into white contour lines**, because that is what water does to anything seen through it. The break at the waterline is the mark, and it reads the hero story: what is under the surface is not where you expect it. Designed on the Paper canvas (**Divings Design System → page "03 Guide mark" → artboard "Guide mark — making it specific", concept H3**) and exported from there — **Paper is the source; re-export rather than hand-editing the SVG.** The submerged contour is white, not safety orange: orange on steel is 3.9:1 and collapses into a brown smear at 16px, where white is 16.6:1. Replaced the staircase (2026-08-20), which was rejected as reading "I did it" when the guide says "you can do it", and then as a generic icon. Plan R16's line about the v1 constellation is long out of date. PNG fallbacks are generated — never hand-edit them, run `node scripts/build-icons.js`.
...
node scripts/build-icons.js          # run after any change to assets/mark.svg — regenerates the favicons + apple-touch icon (needs local Chrome)
